...
public class Aircraft {
    ...
    public Aircraft(int nRows, int nSeatsPerRow, int[] aisleSeat, int[] emergencyRow) {
        try {
            // number of rows
            if (nRows < 3)
                throw new Exception("Aircraft: the aircraft needs at least 3 rows!");
            this.nRows = nRows;

            // number of seats per row
            if (nSeatsPerRow < 2)
                throw new Exception("Aircraft: the aircraft needs to have at least 2 seats per row!");
            this.nSeatsPerRow = nSeatsPerRow;

            // number of aisles (from 1 aisle, there are 2 corresponding seats
            if (aisleSeat.length < 2)
                throw new Exception("Aircraft: the aircraft needs to have at least 1 aisle!");
            ...
            if (emergencyRow.length < 2)
                throw new Exception("Aircraft: the aircraft needs at least 2 security exists!");
            this.nEmergencyRows = emergencyRow.length;

            // location of emergency rows
            this.emergencyRow = new int[this.nEmergencyRows];
            for (int i = 0; i < this.nEmergencyRows; i++)
                this.emergencyRow[i] = emergencyRow[i];
    ...
    public boolean isSeatEmpty(int row, int col) {
        try {
            if (!this.isRowInBounds(row) || !this.isColInBounds(col))
                throw new Exception(
                        "Aircraft: impossible to verify whether the seat is empty or not: seat coordinates out of bounds");
        } catch (Exception e) {
            e.printStackTrace();
            System.exit(1);
        }
